...
public class TempDefaultMob : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        combinedLayerMask = CreateLayerMask("Player", "Tower");
        Init();
    ...
    protected virtual void Idle()
    {
        if (!orderInit)
        {
            waitTime = Time.fixedTime + Random.Range(0.05f, 0.25f); // 랜덤 딜레이
    ...
    protected virtual void Attack()
    {
        ...
        //    }
        //    else if (target.transform.CompareTag("Tower"))
        //    {
        //        // 타워 관련 코드가 아직 없음
        //    }
    ...
    public virtual void GainDamage(int _damage)
    {
        ...
        //    hitDamage = absorbedDamage / (armor + 1);
    ...
    protected virtual void Die()
    {
        ...
        {
            //GameManager.instance.destroyMobCount++;
            //Debug.Log($"잡은 몹수: {GameManager.instance.destroyMobCount}");


            // 애니메이션 종료
            if(PoolManager.Instance == null)
                Debug.Log("NULL PoolManager");
            if (this.gameObject == null)
                Debug.Log("NULL gameObject");
            PoolManager.Instance.ReturnMob(this.gameObject);
            //gameObject.SetActive(false);

            // 오브젝트가 파괴될 때 리스트에서 제거
            /*
            if (gameManager.currentSpawnedMobsInField.Contains(gameObject))
            {
                gameManager.currentSpawnedMobsInField.Remove(gameObject);
            }
            */
        }
    ...
    void AttackCheck(float distanceToTarget) 
    {
        ...
        // distanceToTarget 플레이어와 적 캐릭터 간의 거리 계산
        //if (target.transform.CompareTag("Player"))
        //{
        //    // 플레이어가 죽었다면 빵집으로 간다
        ...
        //else if (target.transform.CompareTag("Bakery"))
        //{
        //    if (target.gameObject.GetComponent<TempPlayerController>().health <= 0)
        //    {
